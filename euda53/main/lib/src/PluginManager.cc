...
namespace eudaq {
  ...
  StandardEvent PluginManager::ConvertToStandard( const DetectorEvent &dev ) {
    ...
    if( 0 )
      std::cout << "PluginManager::ConvertToStandard 1st dev.NumEvents() " << dev.NumEvents()
		<< std::endl << std::flush;
    ...
    for( size_t i = 0; i < dev.NumEvents(); ++i ) {
      ...
      if( 0 )
	std::cout << "  PluginManager::ConvertToStandard i " << i
		  << std::endl << std::flush;
    ...
    if( 0 )
      std::cout << "PluginManager::ConvertToStandard 2nd dev.NumEvents() " << dev.NumEvents()
		<< std::endl << std::flush;
