...
vector <cluster> getClusq( vector <pixel> pb, int fCluCut = 1 ) // 1 = no gap
{
  ...
  vector <cluster> vc;
  vector <pixel> pbclean;
  for(int i =0;i<pb.size();i++)
  {
	  //if (pb[i].tot <15)
	  	pbclean.push_back(pb[i]);

  }


  if( pbclean.size() == 0 ) return vc;


  int* gone = new int[pbclean.size()];
  for( unsigned i = 0; i < pbclean.size(); ++i )
    gone[i] = 0;

  unsigned seed = 0;

  while( seed < pbclean.size() ) {

    // start a new cluster

    cluster c;
    c.vpix.push_back( pbclean[seed] );
    gone[seed] = 1;
    ...
    do {
      ...
      for( unsigned i = 0; i < pbclean.size(); ++i ) {		
        if( !gone[i] ){ // unused pixel
          for( unsigned int p = 0; p < c.vpix.size(); ++p ) { // vpix in cluster so far
            int dr = c.vpix.at(p).row - pbclean[i].row;
            int dc = c.vpix.at(p).col - pbclean[i].col;
            if( (   dr>=-fCluCut) && (dr<=fCluCut) 
		&& (dc>=-fCluCut) && (dc<=fCluCut) ) {
              c.vpix.push_back(pbclean[i]);
	      gone[i] = 1;
              growing = 1;
              break; // important!
            }
    ...
    while( ( ++seed < pbclean.size() ) && gone[seed] );
...
//RUDY: QUA PUOI AGGIUNGERE TAGLI ALLA CARICA PER PRENDERE I VALORI CENTRALI DELLA LANDAU E MIGLIORARE LA RISOLUZIONE
	...
	for( size_t ipix = 0; ipix < plane.HitPixels( frm ); ++ipix ) {
	  ...
	  if( ipl == iDUT ) {
		...
		px.row = 2*iy + 1; // see ed53 for shallow angle
	      if( chip0 == 182 || chip0 == 211 || chip0 == 512 || chip0 == 793350 || chip0 == 792125 || chip0 == 1001 ) { // HLL
		if( ix%2 )
		  px.row = 2*iy + 1;
		else
		  px.row = 2*iy + 0;
	...
	*/
	...
	if( fabs(dx) < sixcut && fabs(dy) < sixcut ) {
	  ...
	  if( chip0 == 182 || chip0 == 211 || // fresh: no box
	      chip0 == 501 || chip0 == 504 ||
	      chip0 == 520 || chip0 == 524 || chip0 == 531 || chip0 == 543 || chip0 == 550 || chip0 == 793350 || chip0 == 792125 ) {
