...
int main( int argc, char* argv[] )
{
  ...
  TProfile2D roadtotrowpercellHisto( "roadtotrowpercell",
		      "tot rows in track road;filled rows;tracks",
          40, -0.1, 0.1, 80,0.0,0.1);
  ...
  vector<vector<int>> pixelActive(264, vector<int>(384,0)); //for 50x50
    ...
    for( int row = 0; row < 384/(2-fifty); ++row ) {
      ...
      if (pixelActive[col][row] > 0){
        ++nrows;
        controlForDeltaRays.Fill(dy,ymod100);

        if( fabs( dy ) < 0.100 )
          totrows = totrows+pixelActive[col][row];
      ...
      roadtotrowpercellHisto.Fill(depth,ymod100,totrows);
