...
int main( int argc, char* argv[] )
{
  ...
  TH1I roadncolHisto( "roadncol",
		      ...
		      150, 0.5, 150.5 );
  TH2F controlForDeltaRays("rowdy","deltay distribution tracks - row",100,.05,.05,40,0.0,0.1);          
  TProfile2D roadtotrowHisto( "roadtotrow",
		      "tot in nearest row to the track; depth ;min(track - row) distance",
100, -0.22, 0.22, 100,-.05,.05);
TProfile2D roadtotAllrowsHisto( "roadtotAllrows",
		      "tot in all the nearby rows; depth ; track - row distance",
100, -0.22, 0.22, 100,-1.,1.);
    TProfile2D roadtotrowpercellHisto( "roadtotrowpercell",
                  "tot in depth per row cell; depth ; track y in a cell",
    200, -0.22, 0.22, 40, 0., .05);
  ...
  TH1I tridHisto( "trid",
		  ...
		  80, -0.2, 0.2 );
      TH1I trackLengthHisto( "trackLength",
		  "in-time track length;length [#cols];in-time triplets",
		  300, 0.5, 300.5 );
  ...
  TProfile pixqvsd( "pixqvsd",
		    ...
		    80, -0.2, 0.2 );
  TProfile2D pixqvsdpercell( "pixqvsdpercell",
		    "PIX column signal vs depth [mm];depth [mm];y track [mm]",
			     80, -0.2, 0.2, 40,0.0,0.05 );
  ...
  vector<vector<int>> pixelActive(264, vector<int>(384,-1)); //for 50x50
  ...
  for( int col = 128/(2-fifty); col < 264/(2-fifty); ++col ) { // Lin
    ...
    int nrows = 0;   
	  ...
	  double dymin = 999;
    double depthAtEntrance =0;
    int nearestRow = 10000;
    double pzRow0;
    double pyRow0;
    bool nHitFound= false;
    vector< pair<double, int>> deltaYandTot(384/(2-fifty), std::make_pair(1000., -1));
    ...
    for( int row = 0; row < 384/(2-fifty); ++row ) {       
        ...
        if (pixelActive[col][row] > 0) 
          deltaYandTot.push_back(make_pair(dy,pixelActive[col][row]));
        ...
        ++nd;
        ++nrows;
        totSumRows+=pixelActive[col][row];
        if( fabs( dx ) < 0.075 )  
            in = 1;   
          
        //if( fabs(dy) < fabs(dymin) ) {
          //deltaY between tracks and nearest Row should be dymin
          dymin = dy;
          nearestRow = row;
          //coming back to the pixel system
          double x1p = co*xA - so*dz; // turn o
          double y1p = yA;
          double z1p = so*xA + co*dz;

          double x2p = x1p;
          double y2p = ca*y1p + sa*z1p; // tilt a

          //double x3p = cf*x2p + sf*y2p; // rot
          double y3p =-sf*x2p + cf*y2p;
         //ymod50 = fmod(8+x3p, 0.05);// [0,0.05] mm We want to see differences between up and down
         ymod50 = fmod(y3p, 0.05);// [0,0.05] mm We want to see differences between up and down
         ymod100 = fmod(y3p, 0.1);// [0,0.1] mm We want to see differences between up and down
        
        roadtotrowpercellHisto.Fill(depthAtEntrance/nd,ymod50,pixelActive[col][row]);
	    ...
	    if( nd ) { // track in depth
	      depthAtEntrance /= nd;
        depth = depthAtEntrance;
	      tridHisto.Fill( depth );
	      if( roadcol[col] ){ 
	        tridlkHisto.Fill( depth );         
  	      pixqvsd.Fill( depth, colq[col] );
          //cout <<"i-th col "<<col<<endl;
          for (uint64_t i=0;i<deltaYandTot.size();i++){
           /*
            if (deltaYandTot[i].second >-1 && fabs(deltaYandTot[i].first) > 0.05){            
              cout <<"dy, tot "<<deltaYandTot[i].first<<" "<<deltaYandTot[i].second<<endl; 
              cout <<"deltayMin, tot nearest row "<<dymin<<" "<<pixelActive[col][nearestRow]<<endl; 
            }              
        
            if (deltaYandTot[i].second >14 ){ 
              cout <<"rows with large tot "<<endl;           
              cout <<"dy, tot "<<deltaYandTot[i].first<<" "<<deltaYandTot[i].second<<endl; 
              cout <<"deltayMin, tot nearest row "<<dymin<<" "<<pixelActive[col][nearestRow]<<endl; 
            } 
            */         
            roadtotAllrowsHisto.Fill(depth,deltaYandTot[i].first,deltaYandTot[i].second);
          }      
          roadtotrowHisto.Fill(depth,dymin,totSumRows/nd); 
           
          
          pixqvsdpercell.Fill(depth,ymod50,colq[col]);          
        }
        }
