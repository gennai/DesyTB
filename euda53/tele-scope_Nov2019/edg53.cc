...
100, -0.22, 0.22, 100,-.2,.2);
  ...
  for( int col = 128/(2-fifty); col < 264/(2-fifty); ++col ) { // Lin
    ...
    for( int row = 0; row < 384/(2-fifty); ++row ) {       
        ...
        if (fabs( dx ) < 0.075)
          if(fabs( dy ) < 0.2)
            if (pixelActive[col][row] > -1)   
              deltaYandTot.push_back(make_pair(dy,pixelActive[col][row])); 
        if( fabs( dx ) > 0.220 ) continue; // depth
        if( fabs( dy ) > 0.07 ) continue; // road
        ymod50 = fmod(8+y3p, 0.05);// [0,0.05] mm We want to see differences between up and down
        ymod100 = fmod(8+y3p, 0.1);// [0,0.1] mm We want to see differences between up and down
...
//        if( fabs( dx ) < 0.075 )  {
        ...
        roadtotrowpercellHisto.Fill(depth,ymod50,pixelActive[col][nearestRow]);
